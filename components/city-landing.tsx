...
import {
  WHY_ADVENTURE_WITH_US,
  type CityContent,
} from "@/lib/city-content";
import { SUPPORT_PHONE, SUPPORT_PHONE_HREF } from "@/lib/locations";
import { getVehiclesByCategoryName } from "@/lib/vehicles";

const WHY_ICONS = [ShieldCheck, Zap, Bike, MessageCircle] as const;

/** One vehicle strip — bikes, cars or scooters. Hidden when nothing matches. */
async function VehicleStrip({
  categoryName,
  heading,
  blurb,
  icon: Icon,
  ...
  const vehicles = await getVehiclesByCategoryName(categoryName, 4);
  if (vehicles.length === 0) return null;

  const id = `${categoryName.toLowerCase()}-rental`;
    ...
    <section aria-labelledby={id} className="scroll-mt-24">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2
            id={id}
            className="mt-2 text-2xl font-extrabold tracking-tight text-neutral-950 uppercase italic sm:text-3xl"
          >
            {heading}
          </h2>
          <p className="mt-1 max-w-xl text-sm text-neutral-600">{blurb}</p>
        </div>
        <Link
          href="/vehicles"
...
export async function CityLanding({ content }: { content: CityContent }) {
  const faqJsonLd = {
    ...
    <main className="flex-1">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-14 px-4 py-10 sm:px-6 lg:gap-20 lg:px-8 lg:py-14">
        <VehicleStrip
          categoryName="Bike"
          icon={Bike}
          heading={`Bike Rental In ${content.city}`}
          blurb="Himalayan-ready motorcycles, serviced before every trip."
        />
        <VehicleStrip
          categoryName="Car"
          icon={Car}
          heading={`Car Rental In ${content.city}`}
          blurb="Self drive cars and 4×4s for high-altitude roads."
        />
        <VehicleStrip
          categoryName="Scooter"
          icon={Zap}
          heading={`Scooter Rental In ${content.city}`}
          blurb="Easy, light and perfect for short valley runs."
        />
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:gap-8">
          ...
          >
            ...
            >
              Rental Pricing In{" "}
              <span className="text-brand">{content.city}</span>
            </h2>
            <p className="mt-2 text-sm text-neutral-600">
